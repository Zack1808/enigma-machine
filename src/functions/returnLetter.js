...
export const encript = (key, rotor1, rotor2, rotor3, pos1, pos2, pos3) => {
  ...
  index = letters.indexOf(
    rotors[rotor1][index + pos1 <= 25 ? index + pos1 : index + pos1 - 25]
  );
  index = letters.indexOf(
    rotors[rotor2][index + pos2 <= 25 ? index + pos2 : index + pos2 - 24]
  );
  index = letters.indexOf(
    rotors[rotor3][index + pos3 <= 25 ? index + pos3 : index + pos3 - 23]
  );
  ...
  index = rotors[rotor2].indexOf(
    letters[index - pos3 >= 0 ? index - pos3 : index - pos3 + 23]
  );
  index = rotors[rotor1].indexOf(
    letters[index - pos2 >= 0 ? index - pos2 : index - pos2 + 24]
  );
  return letters[index - pos1 >= 0 ? index - pos1 : index - pos1 + 25];
};
